		...
		try {
		    ...
		    while(rs.next()) {
		    	...
		    	mr = new MedicalRecord(rs.getInt("record_id"), rs.getInt("patient_id"), rs.getString("start_date"), rs.getString("end_date"), 
		    			rs.getString("diagnosis"), rs.getString("prescription"), rs.getInt("responsible_doctor"));
...
//                    "VALUES ("+ patient_id +", '"+ start_date+"', '"+end_date+"', '"+diagnosis+"', '"+prescription+"', "+responsible_doctor+")");
		    ...
		    st.executeUpdate("INSERT INTO MedicalRecords(patient_id, start_date, end_date, diagnosis, prescription, responsible_doctor) " +
		                       "VALUES ("+ patient_id +", '"+ start_date+"', '"+end_date+"', '"+diagnosis+"', '"+prescription+"', "+responsible_doctor+")");
	...
	public static Boolean updateMedicalRecord(Integer recordId, Integer patient_id, String start_date,String end_date, String diagnosis, String prescription, Integer responsible_doctor){
		try {
		    ...
		    st.executeUpdate("UPDATE MedicalRecords SET patient_id = "+ patient_id +", start_date = '"+ start_date +"', end_date = '"+ end_date +"', diagnosis = '"+ diagnosis 
		    					+"', prescription = '"+ prescription +"', responsible_doctor = "+ responsible_doctor +" WHERE record_id = " + recordId );
